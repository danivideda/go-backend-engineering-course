...
func updatePost(postID int, p UpdatePostPayload, wg *sync.WaitGroup, tag string) {
	defer wg.Done()
	...
	fmt.Printf("Update response status (%s): %s\n", tag,resp.Status)
}
...
func main() {
	...
	go updatePost(postID, UpdatePostPayload{Title: &title}, &wg, "title")
	go updatePost(postID, UpdatePostPayload{Content: &content}, &wg, "content")
	wg.Wait()
}
